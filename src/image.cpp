...
void Image::drawCircleRight(int x, int y, int radius, Color c) {
	for (int i = x+1; i < radius + x + 1; i++) {
		for (int j = y - radius; j < radius + y + 1; j++) {
			float dx = i - x;
			float dy = j - y;
...
void Image::drawCircleLeft(int x, int y, int radius, Color c) {
